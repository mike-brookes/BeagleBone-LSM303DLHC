...
/**
 \brief BBBLSM303DLHC : A class that provides control of the accelerometer BBBLSM303DLHC and extends I2C devices on the Beaglebone Black.
...
LSM303DLHC::LSM303DLHC( ) {
    ...
    if( this->SetRange( _range::HIGHRES_16G ) < 0 ) {
        cerr << "Critical Error 1 : BBBLSM303DLHC - Attempted to set the Accelerometers Range to : " << this->AccelRange << " | This action failed." << endl;
        exit( 1 );
    }

    if( this->SetPower( _switch::ON ) < 0 ) {
        cerr << "Critical Error 1 : BBBLSM303DLHC - Attempted to set the Accelerometers Power State to : " << this->AccelSwitch << " | This action failed." << endl;
        exit( 1 );
    }

    if( this->SetDataRate( _data_rate::HZ_3000 ) < 0 ) {
        cerr << "Critical Error 1 : BBBLSM303DLHC - Attempted to set the Magnetometers Data Rate to : " << this->MagDataRate << " | This action failed." << endl;
        exit( 1 );
    }

    if( this->SetOperationMode( _operation_mode::Continuous ) < 0 ) {
        cerr << "Critical Error 1 : BBBLSM303DLHC - Attempted to set the Magnetometers Operation Mode to : " << this->MagOperationMode << " | This action failed." << endl;
        exit( 1 );
    }
...
/**
 ...
 \return <bool>.
 *
bool BBBLSM303DLHC::SetOperationMode( operation_mode _MagOperationMode ) {
 ...
 *
BBBLSM303DLHC::POWER_SWITCH BBBLSM303DLHC::GetPowerStatus( void ) const { return this->AccelSwitch; }
*/
/**
 ...
 *
BBBLSM303DLHC::_range BBBLSM303DLHC::GetRangeSetting( void ) const { return this->AccelRange; }
*/
